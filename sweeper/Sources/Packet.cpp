...
{
	char* data = new char[3 + sizeof(str_cnt)];
	int idx = 0;
	...
	// Packet Type : SPEED_PROF
	data[idx++] = STR_PATH_CNT;

	// Set Data : Angle & Speed Profil
	char * temp = (char*)&str_cnt;
	data[idx++] = temp[3];
	data[idx++] = temp[2];
	data[idx++] = temp[1];
	data[idx++] = temp[0];
...
//{
//   char* data = new char[3 + sizeof(x) + sizeof(y) + sizeof(speed)];
//   int idx = 0;
//
//   // Header : 0xAA, 0x00 설정
//   data[idx++] = 0xAA;
...
{
	char* data = new char[3 + sizeof(x) + sizeof(y)] + sizeof(time);
	int idx = 0;
...
{
	char* data = new char[3 + sizeof(real_speed)];
	int idx = 0;
...
void SendPacketTestHackPos(SOCKET s, float test_ang, float test_speed) {
	char* data = new char[3 + sizeof(test_ang) + sizeof(test_speed)];
	int idx = 0;
...
void SendStonedata(SOCKET s, bool find, float dist, float angle) {
	char* data = new char[3 + sizeof(find) + sizeof(dist) + sizeof(angle)];
	int idx = 0;
...
void RecvPacketSweeping(char* msg, float &swip_p)
{
	PacketCvt tmp;
	int idx = 3;
